...
namespace Framework.Debuffs.Content
{
    public static class DebuffCatalog
    {
        ...
        private static readonly SharedStatic<UnsafeHashMap<FixedString64Bytes, DebuffDefinition>> _map =
            SharedStatic<UnsafeHashMap<FixedString64Bytes, DebuffDefinition>>.GetOrCreate<MapTag>();
        private static readonly SharedStatic<bool> _initialized =
            SharedStatic<bool>.GetOrCreate<InitTag>();

        public static void Register(in DebuffDefinition def)
        {
            EnsureInitialized();
        ...
        public static bool TryGet(in FixedString64Bytes id, out DebuffDefinition def)
        {
            EnsureInitialized();
            if (!_initialized.Data || !_map.Data.IsCreated)
            {
        ...
        {
            var list = JsonConvert.DeserializeObject<DebuffDefinition[]>(json);
            if (list == null) return;
        ...
        {
            if (_initialized.Data) return;
            _map.Data = new UnsafeHashMap<FixedString64Bytes, DebuffDefinition>(32, Allocator.Persistent);
